...
public class Region {
	...
	static String regionIndividual = "";
	...
	public static void populatePatientData() {
		...
		String slqRequest = "SELECT region.id, region.nom_fr FROM public.region";
	...
	try {
		while (queryResult.next()) {
			...
			regionIndividual += IndividualHelper.genIndividual
					("http://purl.obolibrary.org/obo/bfo.owl#RegionalHealthDepartment", 
							queryResult.getString("id"));
			// Add region-vous honore
			regionIndividual += IndividualHelper.genDataPropertyAssertion
					("hasName", queryResult.getString("id"), "string", queryResult.getString(("nom_fr")));
	...
	System.out.println(regionIndividual);
